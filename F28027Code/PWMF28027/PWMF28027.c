...
void main(void)
{
    ...
    //PieVectTable.TINT1 = &cpu_timer1_isr;  // CPU Timer 1 used to trigger ADC
    ...
    ConfigCpuTimer(&CpuTimer1, 60, 500);  // Used to trigger ADC every 0.5ms
    ...
    EPwm1Regs.TBCTL.bit.CTRMODE = 0;      //set epwm1 to upcount mode
...
#warn add RCserv1,2,3,4 later
    ...
    if (insideCommand == 0) {
        ...
        if (sendindex >= 3) {
            ...
            EPwm2Regs.CMPA.half.CMPA = Pwm2A;
