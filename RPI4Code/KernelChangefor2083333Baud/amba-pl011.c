		     ...
		     struct ktermios *old)
{
	...
	/* Set baud rate */
	printk(KERN_ALERT "baud rate5 = %d \n", baud);
	printk(KERN_ALERT "got here \n");

	/*
	the baud rate is 2000000, hard code the numbers to 2083333
	myuart2[256+9] = 1;
	myuart2[256+10] = 28;//59;  // 28 is 2,083,333  59 is 1,562,500
	4*9=36 = 0x24 IBRD, 4*10=40=0x28 FBRD
	*/ 
	if (baud == 2000000) {
		pl011_write(28, uap, REG_FBRD); 
		pl011_write(1, uap, REG_IBRD);
		printk(KERN_ALERT "modified to 2083333 \n");
	} else {
		pl011_write(quot & 0x3f, uap, REG_FBRD);
		pl011_write(quot >> 6, uap, REG_IBRD);

		printk(KERN_ALERT "quot & 0x3f %d, quot >> 6 %d \n", quot & 0x3f, quot >> 6);
	}
